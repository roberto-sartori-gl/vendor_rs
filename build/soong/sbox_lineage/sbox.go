...
func usageViolation(violation string) {
	...
	fmt.Fprintf(os.Stderr,
		"Usage: sbox_lineage -c <commandToRun> --sandbox-path <sandboxPath> --output-root <outputRoot> [--depfile-out depFile] [--input-hash hash] <outputFile> [<outputFile>...]\n"+
			"\n"+
			"Deletes <outputRoot>,"+
			"runs <commandToRun>,"+
...
func run() error {
	...
	if exit, ok := err.(*exec.ExitError); ok && !exit.Success() {
		return fmt.Errorf("sbox_lineage command (%s) failed with err %#v\n", commandDescription, err.Error())
	} else if err != nil {
		return err
	}
	...
	if !copyAllOutput && len(missingOutputErrors) > 0 {
		...
		errorMessage += "in sbox_lineage command(" + commandDescription + ")\n\n"
